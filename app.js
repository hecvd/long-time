...
window.longTimeApp = function longTimeApp() {
  return {
    ...
    get themeLabel() {
      return { system: "System theme", light: "Light theme", dark: "Dark theme" }[this.themeMode];
    },
    ...
    startClock() {
      if (!this.clockTimer && !document.hidden) {
        this.clockTimer = window.setInterval(() => { this.now = new Date(); }, 60_000);
      }
    },
    ...
    restorePreferences() {
      try { this.filter = localStorage.getItem("long-time:filter") || ""; } catch (_error) { this.filter = ""; }
      this.sortMode = LongTimeLogic.safePreference(localStorage, "long-time:sort", "recent", ["recent", "start-asc", "start-desc", "name-asc", "name-desc"]);
      this.themeMode = LongTimeLogic.safePreference(localStorage, "long-time:theme", "system", ["system", "light", "dark"]);
      try {
        const value = localStorage.getItem("long-time:expanded");
        this.expandedId = /^\d+$/.test(value || "") ? Number(value) : null;
      } catch (_error) { this.expandedId = null; }
    },

    storePreference(key, value) {
      try { localStorage.setItem(key, String(value)); } catch (_error) { return; }
    },
    ...
    rememberFocus() { this.returnFocus = document.activeElement; },
    restoreFocus() { window.setTimeout(() => this.returnFocus?.focus(), 0); },
    announce(message) { this.statusMessage = ""; this.$nextTick(() => { this.statusMessage = message; }); },

    openNewTracker() {
      this.rememberFocus();
      this.trackerForm = { ...emptyTrackerForm(), started_at: localInputValue(new Date()) };
      this.trackerErrors = {};
    ...
    openEditTracker(tracker) {
      this.rememberFocus();
      this.trackerForm = { id: tracker.id, title: tracker.title, description: tracker.description, started_at: localInputValue(tracker.started_at) };
      this.trackerErrors = {};
      this.trackerFormError = "";
      this.$refs.trackerDialog.showModal();
    },
    ...
    async saveTracker() {
      ...
      try {
        await api(editing ? `/api/trackers/${this.trackerForm.id}` : "/api/trackers", {
          ...
          body: { title: this.trackerForm.title, description: this.trackerForm.description, started_at: apiTimestamp(this.trackerForm.started_at) },
        });
        this.$refs.trackerDialog.close();
      ...
      } finally { this.savingTracker = false; }
    },

    toggleTracker(id) { this.expandedId = this.expandedId === id ? null : id; },

    requestTrackerDelete(tracker) {
      this.rememberFocus();
      this.confirmation = { kind: "tracker", id: tracker.id, name: tracker.title };
      this.$refs.confirmDialog.showModal();
    },

    openNewNote(tracker) { this.openNewEntry(tracker, "note"); },
    openNewMilestone(tracker) { this.openNewEntry(tracker, "milestone"); },

    openNewEntry(tracker, kind) {
      this.rememberFocus();
      this.activeTrackerId = tracker.id;
      this.entryForm = { ...emptyEntryForm(), kind, occurred_at: localInputValue(new Date()), target_at: localInputValue(new Date()) };
      this.entryErrors = {};
      this.entryFormError = "";
    ...
    async saveEntry() {
      ...
      const payload = { kind: this.entryForm.kind, title: this.entryForm.title, body: this.entryForm.body };
    ...
    async confirmDelete() {
      this.deleting = true;
      try {
        const endpoint = this.confirmation.kind === "tracker" ? `/api/trackers/${this.confirmation.id}` : `/api/entries/${this.confirmation.id}`;
    ...
    elapsed(tracker) { return LongTimeLogic.elapsedParts(tracker.started_at, this.now); },
    orderedEntries(tracker) { return LongTimeLogic.orderedEntries(tracker); },
    entryTimingLabel(tracker, entry) { return LongTimeLogic.entryTimingLabel(tracker, entry, this.now); },
    entryIsFuture(tracker, entry) {
      const timestamp = entry.kind === "note" ? new Date(entry.occurred_at) : LongTimeLogic.resolvedMilestoneTarget(tracker, entry);
